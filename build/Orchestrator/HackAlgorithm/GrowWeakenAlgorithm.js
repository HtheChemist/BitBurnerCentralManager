...
export function GrowWeakenAlgorithm(ns, currentHack, hackedHost, availableThreads) {
    ...
    for (let host of hackedHost) {
        ...
        const percentHacked = growThreads ? threadsRatio.growThreads / growThreads : 1;
        const hackAmount = host.maxMoney * percentHacked * MONEY_HACKING_TARGET_PERCENT;
        ...
        potentialHack.push(new Hack(host.name, hackTime, hackAmount, // We aim for 95%
        0, threadsRatio.growThreads, threadsRatio.weakenThreads, hackAmount / hackTime * baseHackChance, HackType.growWeakenHack, baseHackChance));
    }
